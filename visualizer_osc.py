...
def osc_in_handler(address: str, *args: List[Any]) -> None:
    
    if address == "/osc_control/v1":
        # osc_reset = bool(args[0])
        hands_vec[0] = bool(args[0])

    if address == "/osc_control/chan0":
        # osc_speed = float(args[0])
        hands_palm_pos[0][0] = float(args[0])

    if address == "/osc_control/chan1":
        # osc_psi = float(args[0])
        hands_palm_pos[0][1] = float(args[0])
    ...
    if count_size[0] % 5000 == 0:
        current_time = datetime.datetime.now()
        print(f"Setting global values: {hands_vec[0]}. ---> {count_size[0]} / {current_time}")
...
dispatcher.map("/osc_control*", osc_in_handler)
...
def csv_setup(viz):
    ...
    for i in range(2):
        row = []
        for j in range(len(viz.seeds_data.iloc[0])):
            row.append(viz.seeds_data[j][i])
        
        print(row)
        viz.latent_widget.seeds.append(row)
...
def osc_control_in(viz):
    ...
    data_len = len(viz.latent_widget.seeds[0])
    x_val = float(viz.latent_widget.latent.x) % data_len
    x_remap = x_val / data_len

    viz.osc_widget.values.a = x_val
    viz.osc_widget.values.b = x_remap
    viz.osc_widget.values.c = viz.latent_widget.latent.y

    if viz.osc_widget.listen_osc:
        viz.osc_widget.params.b = hands_palm_pos[0][0]
        viz.osc_widget.params.c = hands_palm_pos[0][1]
        if hands_vec[0]:
            viz.latent_widget.latent.x = 0
         


        # viz.osc_widget.params.b = osc_speed
        # viz.osc_widget.params.c = osc_psi
        # if viz.osc_reset == True:
        #     viz.latent_widget.latent.x = 0

    # update values
    inertia = viz.osc_widget.params.a
    speed_mult = viz.osc_widget.params.b
    psi_mult = viz.osc_widget.params.c
    y_mult = viz.osc_widget.params.d
...
async def loop(viz):
    while not viz.should_close():
        if viz.osc_widget.listen:
            osc_control_hands(viz)
        else:
            osc_control_in(viz)
